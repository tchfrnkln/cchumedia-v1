...
export default function ShopPage() {
  ...
  products = products.filter(p => p.basePrice <= priceMax);
    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 py-6">
        <div className="max-w-[1380px] mx-auto px-6">
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-2">
            ...
            <span>›</span>
            <span className="text-gray-700 dark:text-gray-200 font-medium">{search ? `Search: "${search}"` : catLabel}</span>
          </div>
          <div className="flex items-end justify-between flex-wrap gap-4">
            <div>
              <h1 className="font-display font-black text-2xl">{search ? `Results for "${search}"` : catLabel}</h1>
      ...
      <div className="max-w-[1380px] mx-auto px-6 py-8">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-3 mb-6">
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={() => navigate('shop', { cat: c.id })}
              className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${c.id === cat ? 'bg-brand text-white border-brand' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 hover:border-brand hover:text-brand'}`}>
        ...
        <div className="flex gap-6">
          ...
          <div className={`${showFilters ? 'fixed inset-0 z-50 bg-white dark:bg-gray-950 p-6 overflow-y-auto' : 'hidden'} lg:block lg:static lg:z-auto lg:bg-transparent lg:p-0 lg:overflow-visible`}>
            {showFilters && (
              <button onClick={() => setShowFilters(false)} className="flex items-center gap-2 mb-4 text-sm font-bold lg:hidden">
            ...
            <div className="mt-4 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4 hidden lg:block">
              ...
              <input type="range" min={500} max={200000} step={500} value={priceMax}
          ...
          <div className="flex-1 min-w-0">
            {products.length === 0 ? (
              <div className="text-center py-20">
                ...
                <h3 className="font-display font-black text-xl mb-2">No products found</h3>
                <p className="text-gray-400 text-sm mb-6">Try adjusting your filters or search terms</p>
                <Button onClick={() => navigate('shop', { cat: 'all' })}>View All Products</Button>
              </div>
            ) : (
