...
export const useStore = create((set, get) => ({
  ...
  // ── NAVIGATION ─────────────────────────────────────────────
  navigate(page, params = {}, newTab = false) {
    ...
    if (newTab) {
      ...
      // Normal navigation (current tab)
      set({ route: { page, params }, modal: null });
      window.scrollTo({ top: 0, behavior: 'smooth' });

      // Optional: Update URL in address bar
  ...
  addToCartFront(product, config = {}, qty = 1, totalPrice, designData) {
